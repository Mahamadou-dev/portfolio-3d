// app/api/content/skills/[slug]/route.ts
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json({ error: 'Competence introuvable' }, { status: 404 });
    }

    const body = await req.json();
    const doc = normalizeSkill({ ...existing, ...body }, existing);
    ...
    console.error('[skills] mise a jour echouee', error);
...
export async function DELETE(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    console.error('[skills] suppression echouee', error);
